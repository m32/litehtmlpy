...
litehtml::uint_ptr container_cairo_pango::surface(int width, int height) {
    ...
    m_temp_surface	= cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, height);
	m_temp_cr = cairo_create(m_temp_surface);
    return (litehtml::uint_ptr)m_temp_cr;
}

void container_cairo_pango::save(const char *fname) {
    cairo_surface_write_to_png(m_temp_surface, fname);
}
